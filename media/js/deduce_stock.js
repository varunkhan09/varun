...
var add_stock_date = "";
var original_deduce_stock_serialized = "";
...
if(this_page_url.indexOf("deduce_stock.php") >= 0)
{
	...
	$(function(){
		$.ajax({
			type:"POST",
			url:"/app/module/stock/deduce_stock_backend.php",
			data:
			{
	...
	$(document).ready(function(){
		...
		$(document).on('click', '#deduce_stock_b', function(){
			var local_counter = 1;
			var string = "";
			var add_stock_array = [];
			var temp_item_id = "";
			...
			orderid = $("#order_id_t").val();
			if(orderid == "")
			{
				$("#order_id_t").focus();
				$("#dialog-confirm-msg").html("Please enter an Order ID.");
					$("#dialog-confirm-msg").dialog({
						resizable: false,
						modal: true,
						title:"Error",
			...
			$.ajax({
				...
				success:function(message)
				{
					...
					{
						$("#dialog-confirm-msg").html("Order Item detail(s) saved.");
							$("#dialog-confirm-msg").dialog({
								resizable: false,
								modal: true,
					...
					{
						$("#dialog-confirm-msg").html("Could not save Order Details.");
							$("#dialog-confirm-msg").dialog({
								resizable: false,
								modal: true,
								title:"Error",
		...
		$(document).on('click', '#load_deduce_stock_details_b', function(){
			add_stock_date = $("#input_date_t").val();
			...
			$.ajax({
				...
				success:function(message)
				{
					...
					else
					{
						...
						{
							$("#dialog-confirm-msg").html("There is no deduce data for this date.");
								$("#dialog-confirm-msg").dialog({
									resizable: false,
									modal: true,
									title:"Error",
						...
						else
						{
							...
							console.log(original_deduce_stock_serialized);
						}
					}
				}
			});
		...
		$(document).on('click', '#update_deduce_stock_b', function(){
			var updated_deduced_stock_serialized = $("#edited_deduced_item_form").serialize();
			updated_deduced_stock_serialized = updated_deduced_stock_serialized.replace(/&/g, ",");
			console.log(updated_deduced_stock_serialized);
			...
			$.ajax({
				...
				{
					mode:'update_details',
					type_of:"deduce_stock",
					date:add_stock_date,
					original_deduced_item_data:original_deduce_stock_serialized,
					edited_deduced_item_log:updated_deduced_stock_serialized
				},
