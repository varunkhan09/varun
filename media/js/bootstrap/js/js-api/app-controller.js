...
myshop.controller('DeliveryChargeCtrl', function($routeParams,$scope,$log,$window,DELIVERY_ZONE) {
...
myshop.controller('DeliveryChargeEditCtrl', function($routeParams,$scope,$log,$window,DELIVERY_ZONE) {
   /* If shop_id & user_id coming from route url then */
    if($routeParams.hasOwnProperty('shop_id') &&  $routeParams.hasOwnProperty('user_id')){
        if( typeof $routeParams.user_id === 'string'){
             $scope.user_id = parseInt($routeParams.user_id,10);
        ...
        if( typeof $routeParams.shop_id === 'string'){
            DELIVERY_ZONE.getShopDeliveryCharge($routeParams.shop_id).then(function(response){
               angular.forEach(response.data, function(item) {
                   if(item.hasOwnProperty('attribute_code')){
                    if(item.attribute_code === "regular_delivery_charge"){
                       $scope.regular_delivery_charge = item.value;
                    }
                    if(item.attribute_code === "midnight_delivery_charge"){
                       $scope.midnight_delivery_charge = item.value;
                    }
                     if(item.attribute_code === "fixedtime_delivery_charge"){
                       $scope.fixedtime_delivery_charge = item.value;
                    }
                   }
               });
            })
...
myshop.controller('DeliveryChargeUpdateCtrl', function($routeParams,$scope,$log,$window,DELIVERY_ZONE) {
    $scope.DeliveryCharge = {};
    $scope.DeliveryCharge.shop_id = null;
    $scope.DeliveryCharge.regular_id = null;
    $scope.DeliveryCharge.midnight_id = null;
    $scope.DeliveryCharge.fixedtime_id = null;
            ...
            DELIVERY_ZONE.getShopDeliveryCharge($routeParams.shop_id).then(function(response){
                if(response.data !== null || response.data !== undefined ){
                     angular.forEach(response.data, function(item) {
                       if(item.hasOwnProperty('attribute_code')){
                        if(item.attribute_code === "regular_delivery_charge"){
                           $scope.DeliveryCharge.regular_delivery_charge = item.value;
                           $scope.DeliveryCharge.regular_id = item.id;
                        }
                        if(item.attribute_code === "midnight_delivery_charge"){
                           $scope.DeliveryCharge.midnight_delivery_charge = item.value;
                           $scope.DeliveryCharge.midnight_id = item.id;
                        }
                         if(item.attribute_code === "fixedtime_delivery_charge"){
                           $scope.DeliveryCharge.fixedtime_delivery_charge = item.value;
                           $scope.DeliveryCharge.fixedtime_id = item.id;
                        }
                       }
                   });
                }
    ...
    $scope.submit = function(){
        if($scope.DeliveryCharge.shop_id !== null){
            //alert($scope.DeliveryCharge.regular_id);
            // if($scope.DeliveryCharge.regular_id === null || $scope.DeliveryCharge.midnight_id === null || $scope.DeliveryCharge.fixedtime_id === null){
            //     alert('Provide valid input and try it again. Your delivery charge is not configured.');
            //     return false;
            // }
            /* To check all fields are undefined or null then error message*/
            if(($scope.DeliveryCharge.midnight_delivery_charge === undefined && $scope.DeliveryCharge.regular_delivery_charge === undefined && $scope.DeliveryCharge.fixedtime_delivery_charge === undefined) ||($scope.DeliveryCharge.midnight_delivery_charge === null && $scope.DeliveryCharge.regular_delivery_charge === null && $scope.DeliveryCharge.fixedtime_delivery_charge === null) ){
                ...
                /* SEND HERE TO DATABASE */
                DELIVERY_ZONE.update_delivery_charge($scope.DeliveryCharge).then(function(response){
...
myshop.controller('DeliveryCtrl', function($routeParams,$scope,$log,$rootScope,ROOT,DELIVERY_ZONE) {
    ...
    if($routeParams.hasOwnProperty('shop_id') &&  $routeParams.hasOwnProperty('user_id')){
        ...
        if( typeof $routeParams.shop_id === 'string'){
          ...
          //  console.log($scope.shop_id);
            // Check Delivery charge of shop is exits or not 

            DELIVERY_ZONE.getShopDeliveryCharge($scope.shop_id).then(function(response){
               if(response.data.length === 0){
                    $scope.delivery_charge_url = ROOT.DOMAIN+"/app/module/myshop/#"+"/user/"+$scope.user_id+"/shop/"+$scope.shop_id+"/dcharge";
               }else{
                    $scope.delivery_charge_url = ROOT.DOMAIN+"/app/module/myshop/#"+"/user/"+$scope.user_id+"/shop/"+$scope.shop_id+"/edit-dcharge";
               }
            })
