...
class Flaberryorder {
    ...
    public function __construct() {
        ...
        $this->shop_img_name = $this->getShopLogoName();
...
//    }
    ...
    /* THIS CODE HAS BEEN ADDED BY VARUN TO FETCH A SHOP'S LOGO IMAGE */
    public function getShopLogo()
    {
        $type = pathinfo($this->shop_img_name, PATHINFO_EXTENSION);
        $data = file_get_contents($this->shop_img_name);
        $dataUri = 'data:image/' . $type . ';base64,' . base64_encode($data);
        return $dataUri;
    }
    /* THIS CODE HAS BEEN ADDED BY VARUN TO FETCH A SHOP'S LOGO IMAGE */
    ...
    public function getShopLogoName()
    {
        ...
        $query = "select shop_logo from pos_shop_entity where entity_id = $shop_id";
        //return $query;
        $result = mysql_query($query);
        mysql_select_db($custom_database);
        $row = mysql_fetch_assoc($result);
        $temp_image_name = $row['shop_logo'];

        $base_path_folder = $_SERVER['DOCUMENT_ROOT'];
        $temp_image_file_name = $base_path_folder."/media/shoplogo/".$temp_image_name;

        return $temp_image_file_name;
    }
    ...
    public function getShopName()
    {
        ...
        mysql_select_db($vendorshop_database);
        $query = "select shop_name from pos_shop_entity where entity_id = $shop_id";
        $result = mysql_query($query);
        $row = mysql_fetch_assoc($result);
        mysql_select_db($custom_database);
