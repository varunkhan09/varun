...
<head>
	...
	<script>
		...
		query = "select distinct orderid from panelorderdetails where (shop_id_created=<?php echo $shop_id; ?> OR shop_id_by=<?php echo $shop_id; ?>) and  (vendor_id is NULL or vendor_id='')";
		range = 20;
			...
			$(document).on('click', '#filter_form_button', function(){
				...
				if(orderid == "")
				{
					if(delivery_date == "")
					{
						...
						$("#dialog-confirm-msg").html("Please select a Delivery Date.");
					...
					{
						query = "select distinct orderid from panelorderdetails where (shop_id_created=<?php echo $shop_id; ?> or shop_id_by=<?php echo $shop_id; ?>)";
						if(delivery_date != "")
						{
				...
				{
					query = "select distinct orderid from panelorderdetails where orderid="+orderid+" and (shop_id_created=<?php echo $shop_id; ?> or shop_id_by=<?php echo $shop_id; ?>)";
		...
		$(document).ready(function(){
			$(document).on('click', '.sendemailbutton', function(){
				...
				{
					$.fancybox({
						'href'              : 'vp_emailContent.php?order_id='+orderid+'&vendor_shop_id='+vendor_id,
						'width'             : '80%',
						'height'            : '60%',
						'autoScale'         : false,
						'transitionIn'      : 'none',
							...
							$.ajax({
								...
								success:function(message)
								{
									...
									else
									{
										...
										$("#dialog-confirm-msg").html("Some unexpected error occurred.");
											$("#dialog-confirm-msg").dialog({
												resizable: false,
												modal: true,
												title:"Error",
				...
				else
				{
					...
					$("#dialog-confirm-msg").html("Please select a Vendor first.");
						$("#dialog-confirm-msg").dialog({
							resizable: false,
							modal: true,
							title:"Error",
										...
										else
										{
											...
											else
											{
												if(message == "\n\n-1")
												{
													...
													$("#dialog-confirm-msg").html("Some error has occurred while performing operation.");
														$("#dialog-confirm-msg").dialog({
															resizable: false,
															modal: true,
															title:"Error",
		...
		function state_clicked(state)
		{
			...
			switch(state)
			{
				case "non_forwarded":
				{
					query = "select distinct orderid from panelorderdetails where (vendor_id is NULL or vendor_id=0 or vendor_id is NULL) and shop_id_created=<?php echo $shop_id; ?>";
					break;
				}

				case "unacknowledged":
				{
					query = "select distinct orderid from vendor_processing where state=0 and orderid in (select distinct orderid from panelorderdetails where shop_id_created=<?php echo $shop_id; ?> or shop_id_by=<?php echo $shop_id; ?>)";
					break;
				}

				case "acknowledged":
				{
					query = "select distinct orderid from vendor_processing where state=1 and orderid in (select distinct orderid from panelorderdetails where shop_id_created=<?php echo $shop_id; ?> or shop_id_by=<?php echo $shop_id; ?>)";
					break;
				}

				case "shipped":
				{
					query = "select distinct orderid from vendor_processing where state=2 and orderid in (select distinct orderid from panelorderdetails where shop_id_created=<?php echo $shop_id; ?> or shop_id_by=<?php echo $shop_id; ?>)";
					break;
				}

				case "delivered":
				{
					query = "select distinct orderid from vendor_processing where state=3 and orderid in (select distinct orderid from panelorderdetails where shop_id_created=<?php echo $shop_id; ?> or shop_id_by=<?php echo $shop_id; ?>)";
					break;
				}

				case "cancel_closed":
				{
					query = "select distinct orderid from vendor_processing where (state=4 or state=5) and orderid in (select distinct orderid from panelorderdetails where shop_id_created=<?php echo $shop_id; ?> or shop_id_by=<?php echo $shop_id; ?>)";
					break;
				}
			}
			...
			$.ajax({
				type:"POST",
				url:"<?php echo $base_module_path; ?>/createdorders/vp_vendormain_queryexecuter.php",
				data:
				{
					query:query,
					min:'0',
...
<div class='orders_loader' id='orders_loader'>
	<div class="col-xs-12 order_type_div">
		<div class="col-xs-2">
			<div class="each_order_type_div" state="non_forwarded">
				Non-Forwarded Orders (<?php echo $count_nonforwarded_orders; ?>)
			</div>
		</div>
		...
		<div class="col-xs-2">
			<div class="each_order_type_div" state="shipped">
				Shipped Orders (<?php echo $count_shipped_orders; ?>)
			</div>
		</div>

		<div class="col-xs-2">
			<div class="each_order_type_div" state="delivered">
				Delivered Orders (<?php echo $count_delivered_orders; ?>)
			</div>
		</div>

		<div class="col-xs-2">
			<div class="each_order_type_div" state="cancel_closed">
