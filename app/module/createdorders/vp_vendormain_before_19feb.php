...
else
{
	...
	$query = "select distinct orderid from panelorderdetails where vendor_id is NULL";
	$result = mysql_query($query);
	if($result && mysql_num_rows($result))
	{
		$orders_flag = 1;
...
<div class='base_div'>
	<label class='heading'>Your Orders</label>
...
if($orders_flag == 1)
{
	...
	foreach($orders_details_array_keys as $orders_details_array_inside_foreach)
	{
		...
		echo "<td style='width:4%;' class='checkbox_td' id='checkboxtd_$row_counter'><input type='checkbox' class='print_ch checkbox' id='printch_$orders_details_array_inside_foreach'></td>";
		...
		echo "<form method='POST' action='vp_ordermain.php' id='form_$row_counter' target='_blank' style='margin-bottom:5px;'>";
		...
		*/
		...
		echo "<td class='form_click' id='orderid_$row_counter' style='width:11%; $style'>".$orders_details_array[$orders_details_array_inside_foreach]['vendorname']."<br>$display_label</td>";
		echo "</tr>";
		$row_counter++;
	}
}
