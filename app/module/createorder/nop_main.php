...
{
	mysql_select_db($vendorshop_database);
	$query = "select firstname, lastname, email from pos_customers_entity where shop_id=$shop_id";
	$result_query = mysql_query($query);

	while($row = mysql_fetch_assoc($result_query))
	{
		$result .= $row['firstname']." ".$row['lastname']."(".$row['email'].")"."<>";
	}
	$result = rtrim($result, "<>");
...
<form method='post' style='border:0px; margin:0px; display:inline; float:left;' id='main_form'>
	<div id='main_div' style='float:left; width:86%; margin:140px 0% 10% 6%; background-color:#F1F1F1; padding:0px 10px 0px 10px;'>
		<div id='main_div_row1' style='float:left; width:100%;'>
			...
			<div id='main_div_row1_1' style='float:left; width:33%; padding-top:5px; padding-bottom:5px; text-align:center;' class='hoverable'>
				<b>Employee taking the order : </b><?php echo $user_name; ?>
			</div>

			<div id='main_div_row1_2' style='float:left; width:33%; padding-top:5px; padding-bottom:5px; text-align:center;' class='hoverable'>
				<b>Vendors:</b>
			...
			<div id='main_div_row1_3' style='float:left; width:33%; padding-top:5px; padding-bottom:5px; text-align:center;' class='hoverable'>
				<b>Order Source:</b>
				...
				<input type='radio' id='ordersource_vendor_id' name='ordersource_radio' value='Vendor'><label for='ordersource_vendor_id'>Vendor</label>
			</div>
	...
	<div style='float:left; width:100%; padding:15px 0px 0px 0px; border-bottom:3px solid #00688B;'>
		<div id='main_div_row2' style='float:left; width:48%; border-right:3px solid #00688B; height:400px;' class='hoverable'>
			...
			<div id='main_div_row3_2_2_3_3' style='float:left; width:98%;'>
				<div style='display:inline; float:left; width:18%;'>Email<sup style='color:red; font-size:12px;'>*</sup></div><input type='textbox' name='sender_email' id='sender_email' style='width:50%;' required>
			</div>
		...
		<div id='main_div_row5' style='float:left; width:100%; padding:10px 0px 0px 0px;'>
			<div id='main_div_row5_1' style='float:left; width:100%; text-align:left; background-color:#F1F1F1; margin-bottom:40px;'>
				<table style='width:100%; text-align:center;' id='merchandise_table' cellspacing="1">
					...
					<tr id='product_selector_row_1' class='hoverable product_selector_row_class'>
						<td colspan='2'>
							<input type='button' id='product_selector_custom_1' class='buttons product_selector_custom_class' value='Create Custom Product'>
						</td>
						
						<td colspan='1'>
							<input type='button' id='product_selector_catalog_1' class='buttons product_selector_catalog_class' value='Choose from Catalog'>
						</td>
						<td colspan='3'></td>
					</tr>

					<tr id='product_row_1' class='hoverable product_row_class'>
						<td class='thistd'>
		...
		<div id='main_div_row5point0' style='float:left; text-align:center; width:100%; padding:20px 0px 0px 0px;'>
			Price Adjustment : <input type='number' name='price_adjustment_t' id='price_adjustment_t' value='0' min='0' style='width:70px;' required>
		</div>
		...
		<div id='main_div_row5point1' style='float:left; text-align:center; width:100%; padding:20px 0px 0px 0px;'>
		...
		<div id='main_div_row7' style='float:left; width:100%; padding:10px 0px 20px 0px; text-align:left; text-align:center;'>
...
else
{
	if($_REQUEST['flag'] == '1')
	{
		...
		$price_adjustment = $_REQUEST['price_adjustment_t'];
		//$shop_id_by = $_REQUEST['orderby_dd'];						//Commented because shop_id_by has to be calculated from Vendors Dropdown...

		if($order_type != "Select")
		{
			$shop_id_by = $order_type;
		}
		else
		{
			$shop_id_by = $shop_id;
		...
		for($x=1; $x<=$product_total_number; $x++)
		{
			if(isset($_REQUEST['product_id_'.$x]))
			{
				...
				else
				{
					...
					$custom_product_details_array[$custom_product_counter]['item_details'] = $_REQUEST['custom_product_item_details_'.$x];
		...
		*/
		...
		else
		{
			...
			$customer->setFirstname($sender_first_name);
			$customer->setLastname($sender_last_name);
			$customer->setPassword($password);
		}

		mysql_select_db($vendorshop_database);
		$temp_address = $sender_address_line_1.", ".$sender_address_line_2;
		$temp_date_now = date("Y-m-d");	
		$query = "select customer_id from pos_customers_entity where email='$sender_email' and shop_id=$shop_id";
		$result_main = mysql_query($query);
		//echo mysql_num_rows($result_main);
		if(mysql_num_rows($result_main) == 0)
		{
			$query = "insert into pos_customers_entity (shop_id, firstname, lastname, email, address, city, state, pincode, country, telephone, is_active, created_at) values ($shop_id, '$sender_first_name', '$sender_last_name', '$sender_email', '$temp_address', '$sender_city', '$sender_state', '$sender_postal_code', 'India', '$sender_phone', 1, '$temp_date_now')";
			mysql_query($query);
			//echo $query;
		}
		mysql_select_db($magento_database);
		...
		$query = "insert into panelorderdetails (orderid, productid, productsku, productquantity, productunitprice, productvdescription, order_source, account_type, dod, shippingtype, recipient_pincode, shop_id_created, shop_id_by, price_adjustment) values ";
		foreach($normal_product_ids as $normal_product_ids_inside_foreach)
		{
			...
			//echo "<br>";
			$query .= "(varun, ".$normal_product_ids_inside_foreach.", '".$normal_product_details_array[$normal_product_ids_inside_foreach]['sku']."', ".$normal_product_details_array[$normal_product_ids_inside_foreach]['quantity'].", '".$normal_product_details_array[$normal_product_ids_inside_foreach]['unitprice']."', '".$normal_product_details_array[$normal_product_ids_inside_foreach]['vendordescription']."', '".$order_source."', '".$sender_account_type."', '".$delivery_date."', '".$delivery_type."', '".$recipient_postal_code."', $shop_id, $shop_id_by, $price_adjustment), ";
		}

		//echo "<br><br>VARUN : $original_custom_product_id";

		$counter = 0;
		$query_filling_custom_product_item_details = "insert into pos_order_items_entity (shop_id, order_id, product_id, product_quantity, item_id, item_quantity) values ";
		foreach($custom_product_ids as $custom_product_ids_inside_foreach)
		{
			$custom_product_set_flag = 1;
			...
			if($custom_product->save())
			{
				...
				$temp_custom_product_item_details = $custom_product_details_array[$custom_product_ids_inside_foreach]['item_details'];
				$temp_custom_product_item_details_array = explode("<>", $temp_custom_product_item_details);
				foreach($temp_custom_product_item_details_array as $each_custom_product_item_details)
				{
					$temp_each_custom_product_item_details = explode("|", $each_custom_product_item_details);
					$temp_item_id = $temp_each_custom_product_item_details[0];
					$temp_item_quantity = $temp_each_custom_product_item_details[1];

					$query_filling_custom_product_item_details .= "($shop_id, varun, ".$custom_product_array[$counter]['id'].", ".$custom_product_details_array[$custom_product_ids_inside_foreach]['quantity'].", $temp_item_id, $temp_item_quantity), ";
				}
			...
			$query .= "(varun, ".$custom_product_array[$counter]['id'].", '".$custom_product_array[$counter]['sku']."', ".$custom_product_details_array[$custom_product_ids_inside_foreach]['quantity'].", '".$custom_product_details_array[$custom_product_ids_inside_foreach]['unitprice']."', '".$custom_product_details_array[$custom_product_ids_inside_foreach]['vendordescription']."', '".$order_source."', '".$sender_account_type."', '".$delivery_date."', '".$delivery_type."', '".$recipient_postal_code."', $shop_id, $shop_id_by, $price_adjustment), ";
			$counter++;
		}
		...
		//echo "<hr>$currentTime<hr>";
		//$quote->setCreatedAtDate($currentTime);
		//$quote->setUpdatedAtDate($currentTime);
		//$quote->setCreatedAt($currentTime);
		//$quote->setCreatedAt(Mage::getSingleton('core/date')->gmtDate());
		...
		echo "<input type='hidden' id='created_order_id_hidden' value='$varun_order'>";
		echo "<input type='hidden' id='product_total_number' value='$product_total_number'>";
		for($x=1; $x<=$product_total_number; $x++)
		{
			echo "<input type='hidden' id='product_description_".$x."_hidden' value='".$_REQUEST["product_description_textarea_$x"]."'>";
		}
		echo "<input type='hidden' id='order_special_instructions' value='$delivery_special_instructions'>";
		echo "<input type='hidden' id='order_delivery_type' value='$delivery_type'>";

		if(!is_null($varun_order))
		{
?>
		<script>
		/* THIS CODE RECORDS THIS OPERATION IN NOTIFICATIONS */
			$.ajax({
				...
				data:
				{
					notf_type:'order_created',
					order_id:'<?php echo $varun_order; ?>',
				}
			});
		/* THIS CODE RECORDS THIS OPERATION IN NOTIFICATIONS */
		</script>
<?php
			echo "<br><br>";
			echo "<center>";
			echo "You can forward this order<br>";
			echo "<select id='forward_order_dd' name='forward_order_dd' class='thisbuttonfont'>";
			echo "<option value='' selected='selected'>---Select---</option>";
			mysql_select_db($vendorshop_database);
			/*
			$query_forward = "select email from pos_shop_entity where entity_id = $shop_id";
			$result_forward = mysql_query($query_forward);
			echo mysql_error();
			$row_forward = mysql_fetch_assoc($result_forward);
			echo "<option value='".$row_forward['email']."'>Self</option>";

			$query_forward = "select email from pos_shop_entity where entity_id = $flaberry_self_shop_id";
			$result_forward = mysql_query($query_forward);
			echo mysql_error();
			$row_forward = mysql_fetch_assoc($result_forward);
			echo "<option value='".$row_forward['email']."'>Flaberry</option>";
			
			$query_forward = "select a.status, b.vendor_shop_id, c.shop_name, c.email from pos_add_vendor_entity a left join pos_associated_vendors_entity b on a.entity_id = b.invite_id left join pos_shop_entity c on b.vendor_shop_id = c.entity_id where b.self_shop_id = $shop_id and a.status = 2";
			$result_forward = mysql_query($query_forward);
			echo mysql_error();
			while($row_forward = mysql_fetch_assoc($result_forward))
			{
				echo "<option value='".$row_forward['email']."'>".$row_forward['shop_name']."</option>";
			}
			*/
			...
			echo "</select>";
			echo "<br><br>";
			echo "<input type='button' class='buttons' style='margin-left:0px; margin-bottom:20px;' id='forward_order_b' value='Forward Order'>";
			echo "</center>";
			echo "<form id='forward_form' style='display:none;' method='POST' action='nop_SendEmail.php' target='_blank'></form>";
		}
		...
		$order->setCreatedAtDate($currentTime);
		$order->save();
		$order->setUpdatedAtDate($currentTime);
		$order->save();
		...
		/* THIS CODE CHANGES THIS ORDER'S STATUS TO PROCESSING */
		$order2 = Mage::getModel('sales/order')->loadByIncrementId($varun_order);
		$order2->setData('state', 'processing');
		...
		/* THIS CODE INSERT THIS ORDER IN pos_order_items_entity IF IT HAS A CUSTOM PRODUCT */
		if($custom_product_set_flag)
		{
			mysql_select_db($vendorshop_database);
			$query_filling_custom_product_item_details = str_replace("varun", "$varun_order", $query_filling_custom_product_item_details);
			$query_filling_custom_product_item_details = rtrim($query_filling_custom_product_item_details, ", ");
			//echo "<br>$query_filling_custom_product_item_details";
			mysql_query($query_filling_custom_product_item_details);// or die("<br><br>".mysql_error());
			echo mysql_error();
		}
		/* THIS CODE INSERT THIS ORDER IN pos_order_items_entity IF IT HAS A CUSTOM PRODUCT */
		...
		/* THIS CODE INSERTS THIS ORDER IN PANELORDERDETAILS TABLE */
		mysql_select_db($custom_database);
