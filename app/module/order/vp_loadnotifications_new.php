	...
	if($user == "vendor")
	{
		...
		if(!isset($_REQUEST['markorders']))
		{
			...
			while($row = mysql_fetch_assoc($result))
			{
				...
				$orderid_array[] = $orderid;
			}
		}
		else
		{
			$mark_these_orders = json_decode($_REQUEST['']);
			foreach($mark_these_orders as $mark_each_order)
			{
	...
	else
	{
		if($user == "admin")
		{
			...
			while($row = mysql_fetch_assoc($result))
			{
				...
				{
					//echo "Landed 0.";
					$temp_notification = "<b>$user</b> sent Order <b>$orderid</b> to <b>$vendor_name</b> for Acknowledgement.<br>$time";
				}
				else
				{
					if($type == "1")
					{
						//echo "Landed 1.";
						$temp_notification = "<b>$user</b> has updated the vendor price for Order <b>$orderid</b>.<br>$time";
					}
					else
					{
						if($type == "2")
						{
							//echo "Landed 2.";
							$temp_notification = "<b>$vendor_name</b> has Acknowledged the Order <b>$orderid</b>.<br>$time";
						}
						else
						{
							if($type == "3")
							{
								//echo "Landed 3.";
								$temp_notification = "<b>$vendor_name</b> has Shipped the Order <b>$orderid</b>.<br>$time";
							}
							else
							{
								if($type == "4")
								{
									//echo "Landed 4.";
									$temp_notification = "<b>$vendor_name</b> has Delivered the Order <b>$orderid</b>.<br>$time";
								}
								else
								{
									if($type == "5")
									{
										//echo "Landed 5.";
										$temp_notification = "<b>$vendor_name</b> has Rejected the Order <b>$orderid</b>.<br>$time";
									}
									else
									{
										if($type == "6")
										{
											...
											{
												$temp_notification = "<b>$vendor_name</b> has added Comment '<b>$comment</b>' in Order <b>$orderid</b>.<br>$time";
											}
											else
											{
												$temp_notification = "<b>$admin_name</b> has added Comment '<b>$comment</b>' in Order <b>$orderid</b>.<br>$time";
											}
										}
										else
										{
											if($type == "7")
											{
												...
												{
													$temp_notification = "<b>$vendor_name</b> has added a Remark '<b>$remark</b>' while accepting the Order <b>$orderid</b>.<br>$time";
												}
												else
												{
													$temp_notification = "<b>$admin_name</b> has added a Remark '<b>$remark</b>' while accepting the Order <b>$orderid</b>.<br>$time";
												}
											}
											else
											{
