...
else
{
	if($_REQUEST['flag'] == "1")
	{
		...
		if($row[0] == "") 				//This case handles cases when this order has been rejected by at least one vendor previously...
		{
			...
			$rejected_order = 1;	//Tells that this order was rejected in history, and re-assigned to this shop now...
		}
		else 							//This case handles cases when this order is processd normally, without being rejected...
		{
			...
			$rejected_order = 0;	//Tells that this order is assigned freshly to this shop now...
		}
		$result = mysql_query($query);
?>
		<script>
			$(document).ready(function(){
				...
				if(<?php echo $rejected_order; ?>)		//This code sends different record notification query based on flag that tells this order is se-assigned or not...
				{
					$.ajax({
						type:"POST",
				...
				else
				{
					$.ajax({
						...
						data:
						{
							notf_type:'order_accepted_by_admin',
