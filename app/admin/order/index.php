	...
	<script>
		...
		function state_clicked(state)
		{
			...
			switch(state)
			{
				case "non_forwarded":
				{
					query = "select distinct orderid from panelorderdetails where (vendor_id is NULL or vendor_id=0 or vendor_id is NULL)";
					break;
				}

				case "unacknowledged":
				{
					query = "select distinct orderid from vendor_processing where state=0 and orderid in (select distinct orderid from panelorderdetails)";
					break;
				}

				case "acknowledged":
				{
					query = "select distinct orderid from vendor_processing where state=1 and orderid in (select distinct orderid from panelorderdetails)";
					break;
				}

				case "shipped":
				{
					query = "select distinct orderid from vendor_processing where state=2 and orderid in (select distinct orderid from panelorderdetails)";
					break;
				}

				case "delivered":
				{
					query = "select distinct orderid from vendor_processing where state=3 and orderid in (select distinct orderid from panelorderdetails)";
					break;
				}

				case "cancel_closed":
				{
					query = "select distinct orderid from vendor_processing where (state=4 or state=5) and orderid in (select distinct orderid from panelorderdetails)";
					break;
				}
			}
